...
exports.dropAllIndexes = exports.createOptimizedIndexes = void 0;
...
const createProductIndexes = async (db) => {
    ...
    try {
        ...
        const textIndexes = existingIndexes.filter((index) => index.key && index.key._fts === "text");
        for (const textIndex of textIndexes) {
            if (textIndex.name !== "product_text_search") {
                logger_1.logger.info(`🗑️ Dropping old text index: ${textIndex.name}`);
        ...
        if (error.code === 85 || error.code === 86) {
            ...
            logger_1.logger.warn("⚠️ Text index already exists with different options, skipping...");
        }
        else {
            logger_1.logger.error("❌ Error creating text index:", error.message);
        }
    }
    ...
    logger_1.logger.info("📦 Product indexes created");
};
const createCategoryIndexes = async (db) => {
    ...
    logger_1.logger.info("📂 Category indexes created");
};
const createBrandIndexes = async (db) => {
    ...
    // Text search
    await collection.createIndex({ name: "text", description: "text" }, { background: true });
    logger_1.logger.info("🏷️ Brand indexes created");
};
const createCartIndexes = async (db) => {
    ...
    logger_1.logger.info("🛒 Cart indexes created");
};
const createOrderIndexes = async (db) => {
    ...
    logger_1.logger.info("📋 Order indexes created");
};
const createReviewIndexes = async (db) => {
    ...
    logger_1.logger.info("⭐ Review indexes created");
};
const logIndexStatistics = async (db) => {
    try {
        ...
        for (const collectionName of collections) {
            ...
            logger_1.logger.info(`📊 ${collectionName}: ${indexes.length} indexes created`);
        }
    }
    catch (error) {
        logger_1.logger.warn("Could not log index statistics:", error);
    }
};
    ...
    try {
        ...
        for (const collectionName of collections) {
            try {
                ...
                logger_1.logger.info(`🗑️ Dropped indexes for ${collectionName}`);
            }
            catch (error) {
                logger_1.logger.warn(`Could not drop indexes for ${collectionName}:`, error);
            }
        }
