...
exports.createBrand = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    let slug = baseSlug;
    let counter = 1;
    while (await Brand_1.Brand.findOne({ slug })) {
...
exports.updateBrand = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    if (updateData.name) {
        ...
        let slug = baseSlug;
        let counter = 1;
        while (await Brand_1.Brand.findOne({ slug, _id: { $ne: req.params.id } })) {
            slug = `${baseSlug}-${counter}`;
            counter++;
        }
        updateData.slug = slug;
    }
    const brand = await Brand_1.Brand.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
    if (!brand) {
        return next(new AppError_1.AppError("Brand not found", 404));
    }
