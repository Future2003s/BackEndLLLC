...
exports.bulkUpdateStock = exports.getInventoryProducts = exports.getInventoryAnalytics = exports.updateProductStock = exports.getLowStockProducts = exports.getInventoryOverview = void 0;
 ...
 */
exports.getInventoryOverview = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const cacheKey = "inventory_overview";
    // Try cache first
    let overview = await cacheService_1.cacheService.get("inventory", cacheKey);
    if (!overview) {
        const [totalProducts, lowStockProducts, outOfStockProducts, totalValue, categories, brands] = await Promise.all([
            ...
            Category_1.Category.find({ isActive: true }).select("name"),
            Brand_1.Brand.find({ isActive: true }).select("name")
        ]);
        overview = {
            totalProducts,
            lowStockProducts,
        ...
        // Cache for 5 minutes
        await cacheService_1.cacheService.set("inventory", cacheKey, overview);
    }
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Inventory overview retrieved successfully", overview));
});
/**
...
exports.getLowStockProducts = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Low stock products retrieved successfully", {
        products,
        pagination: {
            page: parseInt(page),
...
exports.updateProductStock = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!product) {
        return res.status(404).json(new apiResponse_1.ApiResponse(true, "Product not found", null));
    }
    ...
    switch (operation) {
        ...
        case "subtract":
            ...
            break;
        default:
            return res.status(400).json(new apiResponse_1.ApiResponse(true, "Invalid operation", null));
    }
    if (newStock < 0) {
        return res.status(400).json(new apiResponse_1.ApiResponse(true, "Stock cannot be negative", null));
    }
    ...
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Stock updated successfully", {
 ...
 */
exports.getInventoryAnalytics = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Inventory analytics retrieved successfully", analytics));
});
/**
 * Get products with advanced filtering
 */
exports.getInventoryProducts = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (category) {
        const categoryDoc = await Category_1.Category.findOne({ name: category });
        if (categoryDoc) {
            filter.category = categoryDoc._id;
        }
    }
    if (brand) {
        const brandDoc = await Brand_1.Brand.findOne({ name: brand });
        if (brandDoc) {
    ...
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Inventory products retrieved successfully", {
        products,
        pagination: {
            page: parseInt(page),
            limit: parseInt(limit),
 ...
 * Bulk update stock
 */
exports.bulkUpdateStock = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { updates } = req.body; // Array of { productId, stock, operation }
    if (!Array.isArray(updates) || updates.length === 0) {
        return res.status(400).json(new apiResponse_1.ApiResponse(true, "Updates array is required", null));
    }
    const results = [];
    const errors = [];
    ...
    // Clear cache
    await cacheService_1.cacheService.delete("inventory", "inventory_overview");
    res.status(200).json(new apiResponse_1.ApiResponse(true, "Bulk stock update completed", {
        results,
        errors,
        summary: {
