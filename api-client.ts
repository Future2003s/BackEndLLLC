...
class ApiClient {
    ...
    constructor(baseURL: string = "https://api.lalalycheee.vn/api/v1") {
...
export class AuthAPI {
    ...
    async register(userData: RegisterRequest): Promise<ApiResponse<AuthResponse>> {
        const response = await this.client.post<AuthResponse>(API_ENDPOINTS.AUTH.REGISTER, userData);
    ...
    async login(credentials: LoginRequest): Promise<ApiResponse<AuthResponse>> {
        const response = await this.client.post<AuthResponse>(API_ENDPOINTS.AUTH.LOGIN, credentials);
    ...
    async resetPassword(token: string, data: ResetPasswordRequest): Promise<ApiResponse<AuthResponse>> {
        const response = await this.client.put<AuthResponse>(`${API_ENDPOINTS.AUTH.RESET_PASSWORD}/${token}`, data);
    ...
    async refreshToken(refreshToken: string): Promise<ApiResponse<RefreshTokenResponse>> {
        const response = await this.client.post<RefreshTokenResponse>(API_ENDPOINTS.AUTH.REFRESH_TOKEN, {
            refreshToken
        });
...
export class UserAPI {
    ...
    async updateAddress(addressId: string, data: UpdateAddressRequest): Promise<ApiResponse<Address[]>> {
        return this.client.put<Address[]>(`${API_ENDPOINTS.USERS.ADDRESS_BY_ID}/${addressId}`, data);
    }
    ...
    async setDefaultAddress(addressId: string): Promise<ApiResponse<Address[]>> {
        return this.client.put<Address[]>(`${API_ENDPOINTS.USERS.SET_DEFAULT_ADDRESS}/${addressId}/default`);
    }
