...
import { uploadToCloudinary, deleteCloudinaryImage, getPublicIdFromUrl } from "../middleware/upload";
import {
    validateImageLimits,
...
export const uploadProductImages = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const currentMaxOrder =
            product.images.length > 0 ? Math.max(...product.images.map((img) => img.order || 0)) : -1;
...
export const updateProductImageMetadata = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const { id: productId } = req.params;
...
export const deleteProductImages = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const { id: productId } = req.params;
